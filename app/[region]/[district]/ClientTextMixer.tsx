...
interface ClientTextMixerProps {
  region: string;
  district: string;
  dongName?: string; // 🌟 1. dongName prop 추가
  shops?: ShopItem[];
}

// 🌟 구/동 페이지 전용: 스팸 키워드 배제 클린 웰니스 패턴 10종
...
export default function ClientTextMixer({ region, district, dongName, shops = defaultShops }: ClientTextMixerProps) {
  // 🌟 동 이름이 있으면 "구로구 구로3동", 없으면 "구로구"로 표기
  const locationText = dongName ? `${district} ${dongName}`.trim() : `${district}`.trim();

  // 깜빡임(FOUC) 없이 지역 명칭 기반 고유 텍스트 생성
    ...
    <div className="space-y-8">
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {shops.map((shop) => {
            // 🌟 2. 동 이름이 있으면 동 샵 상세(/구로구/구로3동/shop/1)로, 없으면 구 샵 상세(/구로구/shop/1)로 자동 분기
