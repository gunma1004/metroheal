...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const descriptionVariants = [
    ...
    `${currentRegion} 스트레스 해소에 특화된 출장마사지 힐링. 지친 심신에 활력을 불어넣는 전문 힐러 방문.`,
  ...
  return {
    ...
    alternates: {
      canonical: `https://metroheal.netlify.app/massage/${resolvedParams.region}/${encodeURIComponent(safeDecode(resolvedParams.district))}/shop/${targetId}`,
    },
    openGraph: {
      title: pageTitle,
      description: pageDescription,
      url: `https://metroheal.netlify.app/massage/${resolvedParams.region}/${encodeURIComponent(safeDecode(resolvedParams.district))}/shop/${targetId}`,
      siteName: "메트로힐",
      locale: "ko_KR",
      type: "article",
    },
    ...
    <div className="bg-[#050505] text-gray-100 min-h-screen flex flex-col font-sans selection:bg-amber-500 selection:text-black pb-28">
      ...
      <header className="sticky top-0 z-50 bg-[#050505]/85 backdrop-blur-xl border-b border-amber-500/20 px-4 py-3.5 shadow-[0_4px_20px_rgba(245,158,11,0.1)]">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          ...
          <Link 
            href={`/massage/${resolvedParams.region}/${encodeURIComponent(safeDecode(resolvedParams.district))}`}
