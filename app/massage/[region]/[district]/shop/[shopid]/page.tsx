...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const descriptionVariants = [
    ...
    `${currentRegion} 내 공간에서 편안하게 누리는 출장 마사지 홈케어. ${shop.cleanName}에서 지금 확인하세요.`,
    ...
    `${currentRegion} 은은한 향기와 함께하는 출장 마사지 아로마. 숙련된 관리사의 디테일한 손길을 경험하세요.`,
    `${currentRegion} 딥릴렉스 테라피로 피로를 날려주는 출장 마사지. 신속하고 안전한 방문 서비스를 제공합니다.`,
    `${currentRegion} 소프트한 감성 터치로 힐링을 드리는 출장 마사지 바디 케어. 지친 하루 끝 완벽한 릴렉싱.`,
  ...
  return {
    ...
    alternates: {
      canonical: `https://metroheal.netlify.app/massage/${resolvedParams.region}/${encodeURIComponent(safeDecode(districtParam))}/shop/${targetId}`,
    },
    openGraph: {
      title: pageTitle,
      description: pageDescription,
      url: `https://metroheal.netlify.app/massage/${resolvedParams.region}/${encodeURIComponent(safeDecode(districtParam))}/shop/${targetId}`,
      siteName: "메트로힐",
      locale: "ko_KR",
      type: "article",
    },
    ...
    <div className="bg-[#050505] text-gray-100 min-h-screen flex flex-col font-sans selection:bg-amber-500 selection:text-black pb-28">
      ...
      <header className="sticky top-0 z-50 bg-[#050505]/85 backdrop-blur-xl border-b border-amber-500/20 px-4 py-3.5 shadow-[0_4px_20px_rgba(245,158,11,0.1)]">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          ...
          <Link 
            href={`/massage/${resolvedParams.region}/${encodeURIComponent(safeDecode(districtParam))}`}
