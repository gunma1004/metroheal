...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  ...
  const currentRegion = parseLocationText(resolvedParams.region, resolvedParams.district, dongName);
  ...
  // [규칙 1] Title: '출장'과 '마사지'가 절대 붙어있지 않고 중간 단어로 분산됨
  const titleVariants = [
  ...
  // [규칙 2] Description: '출장마사지' 키워드가 붙어서 포함
  const descriptionVariants = [
    `${currentRegion} 24시 신속 방문 출장 마사지 전문 ${shop.cleanName}. 선입금 없는 100% 후불제로 안심하고 이용하세요.`,
    `${currentRegion} 전지역 출장 마사지 제휴 안내. 최고급 천연 오일로 전신 피로를 부드럽게 풀어드립니다.`,
    `${currentRegion} 맞춤형 출장 마사지 솔루션. 숙련된 테라피스트의 손길로 지친 일상의 활력을 되찾아보세요.`,
    `${currentRegion} 출장 마사지 예약 가이드. 프라이빗한 공간에서 온전한 쉼과 휴식을 선사하는 ${shop.cleanName}.`,
    `${currentRegion} 방문 힐링 출장 마사지 프로그램. 정직한 정찰제 요금과 신속한 방문 배차 시스템을 제공합니다.`,
    `${currentRegion} 출장 마사지 딥티슈 피로회복 안내. 굳은 근육을 시원하고 부드럽게 이완시키는 전문 바디케어.`,
    ...
    `${currentRegion} 스트레스 해소에 특화된 출장 마사지 힐링. 지친 심신에 활력을 불어넣어 주는 전문 힐러 방문.`,
    ...
    `${currentRegion} 쾌적하고 안심할 수 있는 출장 마사지 서비스. ${shop.cleanName}이 정성을 다해 케어해 드립니다.`,
  ...
  const canonicalUrl = `https://metroheal.netlify.app/massage/${resolvedParams.region}/${encodeURIComponent(safeDecode(districtParam))}${
...
export default async function DongShopDetailPage({ params, searchParams }: PageProps) {
  const resolvedParams = await params;
  const resolvedSearchParams = await searchParams;
    ...
    <div className="bg-[#050505] text-gray-100 min-h-screen flex flex-col font-sans selection:bg-amber-500 selection:text-black pb-28">
      ...
      <header className="sticky top-0 z-50 bg-[#050505]/85 backdrop-blur-xl border-b border-amber-500/20 px-4 py-3.5 shadow-[0_4px_20px_rgba(245,158,11,0.1)]">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          ...
          <Link 
            href={`/massage/${resolvedParams.region}/${encodeURIComponent(safeDecode(districtParam))}${dongName ? `/${encodeURIComponent(dongName)}` : ""}`}
          ...
          >
            ← {dongName || safeDecode(districtParam)} 목록
          </Link>
        </div>
      </header>
